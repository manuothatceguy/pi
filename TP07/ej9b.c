...
int *jugar(int bolillero[], TipoCarton cartones[], int cantJugadores);
...
int main(){
    ...
    do
    {
        num = getint("Ingrese el número de jugadores: ");
    } while (num < 1);
    
    TipoCarton * cartones = malloc(num*sizeof(TipoCarton));
    randomize();
    ...
    int * win = jugar(bolillero,cartones,num);
...
int *jugar(int bolillero[], TipoCarton cartones[], int cantJugadores){
    int bingo = 0, cantidadBolillas = CANT_BOLILLAS, bolilla, *winnersArray = calloc(cantJugadores,sizeof(int));
    int * lineasJug = malloc(sizeof(int)*cantJugadores);
    while(!bingo){
